...
for column in data.columns[:-1]:
  for i in range(len(data[column].values)):
    data[column][i] = np.mean(np.var(data[column][i],axis=0))
  param = data[column]
  stability = data['stability'].values
  # compute correlation of parameter with stability
  correlation_df['Correlation'][column] = compute_correlation(param, stability)
